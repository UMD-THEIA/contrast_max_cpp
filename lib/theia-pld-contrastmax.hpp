...
#include "theia-pld-filereader.hpp"
#include <Eigen/Dense>
#include <cstdint>
#include <vector>
#include <string>
...
// Macro used to suppress unused variable warnings.
#define UNUSED(x) (void)(x)

// Enable Eigen wrappers for the optimizer.
#define OPTIM_ENABLE_EIGEN_WRAPPERS

// Suppress some warnings for third-party includes.
...
image_t create_image(std::vector<event_t> events, int width, int height) {
  ...
  for (event_t event : events) {
    // Discard events that are outside the image.
...
void write_image(std::vector<uint64_t> imgdata, uint64_t width, uint64_t height) {
...
double singlepass_optim(Eigen::VectorXd x0, Eigen::VectorXd *grad_out, void *fd) {
  UNUSED(grad_out);
  if (x0.size() != 3) {
...
double singlepass_optim_blur(Eigen::VectorXd x0, Eigen::VectorXd *grad_out, void *fd) {
  UNUSED(grad_out);
  if (x0.size() != 3) {
    throw std::runtime_error("x0 must have exactly 3 elements");
  }
...
double singlepass_blur(Eigen::Vector3d x0, filedata_t filedata) {
  ...
  image_t image = create_image(warped_events, filedata.metadata.width, filedata.metadata.height);
...
std::vector<event_t> warp_events(std::vector<event_t> events, Eigen::Vector3d theta) {
...
event_t warp_event(event_t event, uint64_t time_int, Eigen::Vector3d theta) {
  ...
  double x = event.x;
  double y = event.y;
  double t = static_cast<double>(time_int) / 1000.0;

  Eigen::Vector3d event_vector{ { x, y, sqrt((1280 * 1280 + 720 * 720) - x * x - y * y) } };
  ...
  Eigen::Vector3d warped_event_vector = trans_inv * theta_hat * trans * event_vector;
